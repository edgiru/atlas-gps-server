...
function saveOptions(res, login, data) {
    ...
    connection.query('SELECT * FROM user WHERE name = ?', [login], function (error, result, fields) {
        if (error) {
            console.log('Data Base Error2: new dev');
            // connection.end();
            res.statusCode = 500;
            ...
            if (getOptions) {
                ...
                getOptions.forEach(function(el, index){
                    if(el['imei']==data.imei){
                        getOptions[index] = null;
                ...
                for(var i = 0; i<getOptions.length; i++){
                    if(getOptions[i]){
                        tmp[index]  = getOptions[i];
                        index ++;
                    }
                }

                setOptions = JSON.stringify(tmp);
                res.end('Устройство удалено');
